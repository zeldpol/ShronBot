...
def sticker(message):
    if message.sticker.is_animated:
        ...
        add_or_delete_sticker(message, png_sticker=None, tgs_sticker=downloaded_file, webm_sticker=None)
    else:
        add_or_delete_sticker(message, png_sticker=message.sticker.file_id, tgs_sticker=None, webm_sticker=None)


def add_or_delete_sticker(message, emoji=None, png_sticker=None, tgs_sticker=None,webm_sticker=None):
    assert bool(tgs_sticker) != bool(png_sticker) != bool(webm_sticker), "Exactly one of tgs_sticker, png_sticker or webm_sticker should be passed" 
    if not emoji:
        emoji = message.sticker.emoji
    if message.sticker.set_name == get_pack_name(message):
        del_sticker(message)
    else:
        add_sticker(message,  emoji=emoji, tgs_sticker=tgs_sticker, png_sticker=png_sticker, webm_sticker=webm_sticker)


def add_sticker(message, emoji, tgs_sticker, png_sticker, webm_sticker):
    pack_name = get_pack_name(message)
    try:
        bot.add_sticker_to_set(message.from_user.id, pack_name, emojis=emoji,  png_sticker=png_sticker, tgs_sticker=tgs_sticker, webm_sticker=webm_sticker)
        ...
        if e.description.strip() == 'Bad Request: STICKERSET_INVALID':
            bot.create_new_sticker_set(message.from_user.id, pack_name, pack_name, emoji, png_sticker=png_sticker, tgs_sticker=tgs_sticker, webm_sticker=webm_sticker)
            bot.send_message(message.chat.id, f"New sticker set created. Sticker added! t.me/addstickers/{pack_name}")
        else:
            raise e
...
def image2sticker(message):
    ...
    add_sticker(message,  emoji="😃", png_sticker=resized_image, tgs_sticker=None, webm_sticker=None)
...
def vn2sticker(message):
    ...
    if code := os.system(f'ffmpeg -i tmp/video_note.mp4 -c copy -map 0 -segment_time {MAX_VN_LEN} -f segment -reset_timestamps 1 tmp/video_note%d.mp4'):       
    ...
    for i in range(int(message.video_note.duration/MAX_VN_LEN)):
        if code := os.system(f'ffmpeg -i tmp/video_note{i}.mp4 -y -vf "crop={ln}:{ln},scale={MAX_IMAGE_SIZE}:{MAX_IMAGE_SIZE}" -c:v libvpx-vp9 -crf 30 -b:v 0 -b:a 128k -loop 0 -an tmp/output{i}.webm'):
            error_hanlder(message, f'BONK, something wrong! ffmpeg returned code {code}')
        else:
            with  open(f'tmp/output{i}.webm', 'rb') as video_file:
                video = video_file.read()
                add_sticker(message,  emoji="😃", png_sticker=None, tgs_sticker=None, webm_sticker = video)

    # if code := os.system(f'ffmpeg -i tmp/video_note.mp4 -y -vf "crop={ln}:{ln},scale={MAX_IMAGE_SIZE}:{MAX_IMAGE_SIZE}" -c:v libvpx-vp9 -crf 30 -b:v 0 -b:a 128k -loop 0 -an tmp/output.webm'):
    #     error_hanlder(message, f'BONK, something wrong! ffmpeg returned code {code}')
    # else:
